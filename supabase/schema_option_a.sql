...
create policy if not exists "users can read own polygons" on public.project_polygons
  for select using (
    exists (
...
create policy if not exists "users can insert own polygons" on public.project_polygons
